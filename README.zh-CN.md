...
Grasp 完全本地运行，使用专属的 `chrome-grasp` browser 配置目录，让智能体拥有持久、可见、可恢复的网页运行时，而不是一次性标签页和单站点脚本。这个专属 profile 是 Grasp 的运行时边界，不等于“你当前随手正在使用的任意本地浏览器窗口”。`v0.6.4` 的核心承诺很简单：给它一个 URL 和任务意图，它先选出最合适的路径，让这个决定可解释，在改页面前先确认运行时边界，并沿着同一条 runtime 路径继续执行。
